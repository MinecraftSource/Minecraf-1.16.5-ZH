...
public class EndAdvancements implements Consumer<Consumer<Advancement>> {
   public void accept(Consumer<Advancement> p_accept_1_) {
      ...
      Advancement advancement3 = Advancement.Builder.builder().withParent(advancement2).withDisplay(Blocks.PURPUR_BLOCK, new TranslationTextComponent("advancements.end.find_end_city.title"), new TranslationTextComponent("advancements.end.find_end_city.description"), (ResourceLocation)null, FrameType.TASK, true, true, false).withCriterion("in_city", PositionTrigger.Instance.forLocation(LocationPredicate.forFeature(Structure.END_CITY))).register(p_accept_1_, "end/find_end_city");
