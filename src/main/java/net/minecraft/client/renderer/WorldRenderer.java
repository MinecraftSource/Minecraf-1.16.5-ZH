...
public class WorldRenderer implements IResourceManagerReloadListener, AutoCloseable {
   ...
   public void loadRenderers() {
      if (this.world != null) {
         ...
         RenderTypeLookup.setFancyGraphics(Minecraft.isFancyGraphicsEnabled());
   ...
   private void setupTerrain(ActiveRenderInfo activeRenderInfoIn, ClippingHelper camera, boolean debugCamera, int frameCount, boolean playerSpectator) {
      ...
      if (!debugCamera && this.displayListEntitiesDirty) {
         ...
         while(!queue.isEmpty()) {
            ...
            for(Direction direction1 : FACINGS) {
               ChunkRenderDispatcher.ChunkRender chunkrenderdispatcher$chunkrender2 = this.getRenderChunkOffset(blockpos1, chunkrenderdispatcher$chunkrender3, direction1);
               if ((!flag || !worldrenderer$localrenderinformationcontainer1.hasDirection(direction1.getOpposite())) && (!flag || direction == null || chunkrenderdispatcher$chunkrender3.getCompiledChunk().isVisible(direction.getOpposite(), direction1)) && chunkrenderdispatcher$chunkrender2 != null && chunkrenderdispatcher$chunkrender2.shouldStayLoaded() && chunkrenderdispatcher$chunkrender2.setFrameIndex(frameCount) && camera.isBoundingBoxInFrustum(chunkrenderdispatcher$chunkrender2.boundingBox)) {
                  WorldRenderer.LocalRenderInformationContainer worldrenderer$localrenderinformationcontainer = new WorldRenderer.LocalRenderInformationContainer(chunkrenderdispatcher$chunkrender2, direction1, worldrenderer$localrenderinformationcontainer1.counter + 1);
      ...
      for(WorldRenderer.LocalRenderInformationContainer worldrenderer$localrenderinformationcontainer2 : this.renderInfos) {
         ...
         if (chunkrenderdispatcher$chunkrender4.needsUpdate() || set.contains(chunkrenderdispatcher$chunkrender4)) {
            ...
            boolean flag1 = blockpos2.distanceSq(blockpos) < 768.0D;
            if (!chunkrenderdispatcher$chunkrender4.needsImmediateUpdate() && !flag1) {
   ...
   public void updateCameraAndRender(MatrixStack matrixStackIn, float partialTicks, long finishTimeNano, boolean drawBlockOutline, ActiveRenderInfo activeRenderInfoIn, GameRenderer gameRendererIn, LightTexture lightmapIn, Matrix4f projectionIn) {
      ...
      boolean flag1 = this.mc.world.func_239132_a_().func_230493_a_(MathHelper.floor(d0), MathHelper.floor(d1)) || this.mc.ingameGUI.getBossOverlay().shouldCreateFog();
      if (this.mc.gameSettings.renderDistanceChunks >= 4) {
         FogRenderer.setupFog(activeRenderInfoIn, FogRenderer.FogType.FOG_SKY, f, flag1);
         iprofiler.endStartSection("sky");
         this.renderSky(matrixStackIn, partialTicks);
      }

      iprofiler.endStartSection("fog");
      FogRenderer.setupFog(activeRenderInfoIn, FogRenderer.FogType.FOG_TERRAIN, Math.max(f - 16.0F, 32.0F), flag1);
   ...
   public void playRecord(@Nullable SoundEvent soundIn, BlockPos pos) {
      ...
      if (soundIn != null) {
         ...
         ISound simplesound = SimpleSound.record(soundIn, (double)pos.getX(), (double)pos.getY(), (double)pos.getZ());
   ...
   public void playEvent(PlayerEntity player, int type, BlockPos blockPosIn, int data) {
      ...
      case 1502:
         ...
         for(int l1 = 0; l1 < 5; ++l1) {
            double d15 = (double)blockPosIn.getX() + random.nextDouble() * 0.6D + 0.2D;
            double d20 = (double)blockPosIn.getY() + random.nextDouble() * 0.6D + 0.2D;
            double d26 = (double)blockPosIn.getZ() + random.nextDouble() * 0.6D + 0.2D;
            this.world.addParticle(ParticleTypes.SMOKE, d15, d20, d26, 0.0D, 0.0D, 0.0D);
         }
         break;
      case 1503:
         ...
         for(int k1 = 0; k1 < 16; ++k1) {
            double d14 = (double)blockPosIn.getX() + (5.0D + random.nextDouble() * 6.0D) / 16.0D;
            double d19 = (double)blockPosIn.getY() + 0.8125D;
            double d25 = (double)blockPosIn.getZ() + (5.0D + random.nextDouble() * 6.0D) / 16.0D;
            this.world.addParticle(ParticleTypes.SMOKE, d14, d19, d25, 0.0D, 0.0D, 0.0D);
         }
         break;
      case 2000:
         ...
         double d18 = (double)blockPosIn.getX() + (double)j1 * 0.6D + 0.5D;
         double d24 = (double)blockPosIn.getY() + (double)j2 * 0.6D + 0.5D;
         double d28 = (double)blockPosIn.getZ() + (double)k2 * 0.6D + 0.5D;
         ...
         for(int i3 = 0; i3 < 10; ++i3) {
            ...
            this.addParticleUnchecked(ParticleTypes.SMOKE, d6, d8, d30, d9, d10, d11);
         }
         break;
      case 2001:
      ...
      case 2007:
         ...
         float f3 = (float)(data >> 16 & 255) / 255.0F;
         float f4 = (float)(data >> 8 & 255) / 255.0F;
         ...
         for(int j = 0; j < 100; ++j) {
            ...
            Particle particle1 = this.addParticleUnchecked(iparticledata, iparticledata.getType().getAlwaysShow(), vector3d.x + d29 * 0.1D, vector3d.y + 0.3D, vector3d.z + d7 * 0.1D, d29, d5, d7);
            if (particle1 != null) {
               float f2 = 0.75F + random.nextFloat() * 0.25F;
               particle1.setColor(f3 * f2, f4 * f2, f5 * f2);
               particle1.multiplyVelocity((float)d23);
            }
         }
      ...
      case 2003:
         ...
         double d17 = (double)blockPosIn.getZ() + 0.5D;

         for(int l2 = 0; l2 < 8; ++l2) {
            this.addParticleUnchecked(new ItemParticleData(ParticleTypes.ITEM, new ItemStack(Items.ENDER_EYE)), d0, d13, d17, random.nextGaussian() * 0.15D, random.nextDouble() * 0.2D, random.nextGaussian() * 0.15D);
         }

         for(double d22 = 0.0D; d22 < (Math.PI * 2D); d22 += 0.15707963267948966D) {
            this.addParticleUnchecked(ParticleTypes.PORTAL, d0 + Math.cos(d22) * 5.0D, d13 - 0.4D, d17 + Math.sin(d22) * 5.0D, Math.cos(d22) * -5.0D, 0.0D, Math.sin(d22) * -5.0D);
            this.addParticleUnchecked(ParticleTypes.PORTAL, d0 + Math.cos(d22) * 5.0D, d13 - 0.4D, d17 + Math.sin(d22) * 5.0D, Math.cos(d22) * -7.0D, 0.0D, Math.sin(d22) * -7.0D);
         }
         break;
      case 2004:
         for(int l = 0; l < 20; ++l) {
            double d12 = (double)blockPosIn.getX() + 0.5D + (random.nextDouble() - 0.5D) * 2.0D;
            double d16 = (double)blockPosIn.getY() + 0.5D + (random.nextDouble() - 0.5D) * 2.0D;
            double d21 = (double)blockPosIn.getZ() + 0.5D + (random.nextDouble() - 0.5D) * 2.0D;
            this.world.addParticle(ParticleTypes.SMOKE, d12, d16, d21, 0.0D, 0.0D, 0.0D);
            this.world.addParticle(ParticleTypes.FLAME, d12, d16, d21, 0.0D, 0.0D, 0.0D);
         }
         break;
      case 2005:
   ...
   public static int getPackedLightmapCoords(IBlockDisplayReader lightReaderIn, BlockState blockStateIn, BlockPos blockPosIn) {
      if (blockStateIn.isEmissiveRendering(lightReaderIn, blockPosIn)) {
         ...
         int k = blockStateIn.getLightValue();
         if (j < k) {
            j = k;
         }
