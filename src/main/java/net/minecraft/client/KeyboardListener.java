...
public class KeyboardListener {
   ...
   private void copyHoveredObject(boolean privileged, boolean askServer) {
      ...
      if (raytraceresult != null) {
         ...
         case BLOCK:
            ...
            if (privileged) {
               if (askServer) {
                  this.mc.player.connection.getNBTQueryManager().queryTileEntity(blockpos, (compound) -> {
                     this.setBlockClipboardString(blockstate, blockpos, compound);
         ...
         case ENTITY:
            ...
            if (privileged) {
               if (askServer) {
                  this.mc.player.connection.getNBTQueryManager().queryEntity(entity.getEntityId(), (compound) -> {
                     this.setEntityClipboardString(resourcelocation, entity.getPositionVec(), compound);
   ...
   public void setupCallbacks(long window) {
      InputMappings.setKeyCallbacks(window, (windowPointer, key, scanCode, action, modifiers) -> {
         this.mc.execute(() -> {
            this.onKeyEvent(windowPointer, key, scanCode, action, modifiers);
         });
      }, (windowPointer, codePoint, modifiers) -> {
         this.mc.execute(() -> {
            this.onCharEvent(windowPointer, codePoint, modifiers);
         });
      });
   }

   public String getClipboardString() {
      return this.clipboardHelper.getClipboardString(this.mc.getMainWindow().getHandle(), (error, description) -> {
         if (error != 65545) {
            this.mc.getMainWindow().logGlError(error, description);
         }
