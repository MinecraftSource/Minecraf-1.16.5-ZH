...
public class ChunkSerializer {
   ...
   private static Map<Structure<?>, StructureStart<?>> func_235967_a_(TemplateManager p_235967_0_, CompoundNBT p_235967_1_, long p_235967_2_) {
      ...
      for(String s : compoundnbt.keySet()) {
         ...
         Structure<?> structure = Structure.NAME_STRUCTURE_BIMAP.get(s1);
         if (structure == null) {
            LOGGER.error("Unknown structure start: {}", (Object)s1);
         } else {
            StructureStart<?> structurestart = Structure.deserializeStructureStart(p_235967_0_, compoundnbt.getCompound(s), p_235967_2_);
   ...
   private static Map<Structure<?>, LongSet> unpackStructureReferences(ChunkPos p_227075_0_, CompoundNBT p_227075_1_) {
      ...
      for(String s : compoundnbt.keySet()) {
         map.put(Structure.NAME_STRUCTURE_BIMAP.get(s.toLowerCase(Locale.ROOT)), new LongOpenHashSet(Arrays.stream(compoundnbt.getLongArray(s)).filter((p_227074_2_) -> {
