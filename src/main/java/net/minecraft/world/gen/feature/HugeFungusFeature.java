...
public class HugeFungusFeature extends Feature<HugeFungusConfig> {
   ...
   public boolean generate(ISeedReader reader, ChunkGenerator generator, Random rand, BlockPos pos, HugeFungusConfig config) {
      Block block = config.field_236303_f_.getBlock();
      ...
      if (blockpos == null) {
         ...
         boolean flag = !config.field_236307_j_ && rand.nextFloat() < 0.06F;
         reader.setBlockState(pos, Blocks.AIR.getDefaultState(), 4);
         this.func_236317_a_(reader, rand, config, blockpos, i, flag);
         this.func_236321_b_(reader, rand, config, blockpos, i, flag);
         return true;
      }
   }
