...
public class GlStateManager {
   ...
   private static final GlStateManager.BooleanState[] LIGHT_ENABLE = IntStream.range(0, 8).mapToObj((texture) -> {
      return new GlStateManager.BooleanState(16384 + texture);
   }).toArray((textureWithLight) -> {
      return new GlStateManager.BooleanState[textureWithLight];
   });
   ...
   private static final GlStateManager.TextureState[] TEXTURES = IntStream.range(0, 12).mapToObj((texture) -> {
      return new GlStateManager.TextureState();
   }).toArray((textureState) -> {
      return new GlStateManager.TextureState[textureState];
   });
