...
public class GLX {
   ...
   private static final Map<Integer, String> LOOKUP_MAP = make(Maps.newHashMap(), (lookupMap) -> {
      lookupMap.put(0, "No error");
      lookupMap.put(1280, "Enum parameter is invalid for this function");
      lookupMap.put(1281, "Parameter is invalid for this function");
      lookupMap.put(1282, "Current state is invalid for this function");
      lookupMap.put(1283, "Stack overflow");
      lookupMap.put(1284, "Stack underflow");
      lookupMap.put(1285, "Out of memory");
      lookupMap.put(1286, "Operation on incomplete framebuffer");
      lookupMap.put(1286, "Operation on incomplete framebuffer");
   });
   ...
   public static LongSupplier _initGlfw() {
      RenderSystem.assertThread(RenderSystem::isInInitPhase);
      MainWindow.checkGlfwError((errorGLFW, errorMessage) -> {
         throw new IllegalStateException(String.format("GLFW error before init: [0x%X]%s", errorGLFW, errorMessage));
      });
      List<String> list = Lists.newArrayList();
      GLFWErrorCallback glfwerrorcallback = GLFW.glfwSetErrorCallback((error, description) -> {
         list.add(String.format("GLFW error during init: [0x%X]%s", error, description));
      });
      if (!GLFW.glfwInit()) {
